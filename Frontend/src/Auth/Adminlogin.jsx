...
function Login() {
  ...
  const handleSendOTP = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/api/auth/admin/send-otp-login`, {
        method: "POST",
  ...
  const handleVerifyOTP = async (otpCode) => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/api/auth/admin/verify-otp-login`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        body: JSON.stringify({
        ...
        <div className="p-6 sm:p-10 flex flex-col justify-center">
          <div className="text-center mb-6">
            <h2 className="text-2xl sm:text-3xl font-bold text-gray-900">
              {step === "credentials" ? "Admin Sign In" : "Verify Email"}
            </h2>
